...
/*
    ...
    O JavaScript possui tanto operadores binários quanto unários e um operador ternário, o operador condicional. Um operador binário exige dois operandos,
    um antes do operador e outro depois:
    ...
    Atribuição bit-a-bit por
    deslocamento á esquerda	                x <<= y	                    x = x << y
    Atribuição bit-a-bit por
    deslocamento á direita                  x >>= y	                    x = x >> y
    Atribuição de bit-a-bit
    deslocamento á direita não assinado	    x >>>= y	                x = x >>> y
    Atribuição AND bit-a-bit	            x &= y	                    x = x & y
    Atribuição XOR bit-a-bit	            x ^= y	                    x = x ^ y
    Atribuição OR bit-a-bit	                x |= y	                    x = x | y

    -----Operadores de comparação-----
    Um operador de comparação compara seus operandos e retorna um valor lógico baseado em se a comparação é verdadeira. Os operandos podem ser numéricos, strings,
    lógicos ou objetos. Strings são comparadas com base em ordenação lexográfica utilizando valores Unicode. Na maioria dos casos, se dois operandos não são do mesmo tipo,
    o JavaScript tenta convertê-los para um tipo apropriado. Isto geralmente resulta na realização de uma comparação numérica. As únicas exceções a esta regra são os
    operadores === e o !==, que realizam comparações de igualdade e desigualdade "estritas". Estes operadores não tentam converter os operandos em tipos compatíveis
    antes de verificar a igualdade. A tabela a seguir descreve os operadores de comparação levando em conta o seguinte código:
    var var1 = 3;
    var var2 = 4;
    ...
    Estritamente
    igual (===)	            Retorna verdadeiro caso os operandos sejam iguais e do
                            mesmo tipo. Veja também Object.is e igualdade em JS.        3 === var1.
    Estritamente
    não igual (!==)	        Retorna verdadeiro caso os operandos não sejam iguais
                            e/ou não sejam do mesmo tipo.                               var1 !== "3". 3 !== '3'.

    Maior que (>)	        Retorna verdadeiro caso o operando da esquerda seja
                            maior que o da direita.	                                    var2 > var1. "12" > 2.

    Maior que
    ou igual (>=)	        Retorna verdadeiro caso o operando da esquerda seja
                            maior ou igual ao da direita.	                            var2 >= var1. var1 >= 3.

    Menor que (<)	        Retorna verdadeiro caso o operando da esquerda seja
                            menor que o da direita.	                                    var1 < var2. "12" < "2".

    Menor que ou
    igual (<=)	            Retorna verdadeiro caso o operando da esquerda seja
                            menor ou igual ao da direita.	                            var1 <= var2. var2 <= 5.

    Nota: (=>) não é um operador, mas a notação para função de seta

    -----Operadores aritméticos-----
    Operadores aritméticos tomam valores numéricos (sejam literais ou variáveis) como seus operandos e retornam um único valor númerico. Os operadores aritméticos
    padrão são os de soma (+), subtração (-), multiplicação (*) e divisão (/). Estes operadores trabalham da mesma forma como na maioria das linguagens de programação
    quando utilizados com números de ponto flutuante (em particular, repare que divisão por zero produz um NaN). Por exemplo:
*/
...
/*
    ...
    Incremento (++)	        Operador unário. Adiciona um ao seu operando. Se usado
                            como operador prefixado (++x), retorna o valor de seu
                            operando após a adição. Se usado como operador
                            pósfixado (x++), retorna o valor de seu operando antes
                            da adição.	                                                Se x é 3, então ++x define x como 4 e retorna 4, enquanto x++
                                                                                        retorna 3 e, somente então, define x como 4.

    Decremento (--)	        Operador unário. Subtrai um de seu operando. O valor
                            de retorno é análogo àquele do operador de incremento.	    Se x é 3, então --x define x como 2 e retorna 2, enquanto x--
                                                                                        retorna 3 e, somente então, define x como 2.

    Negação (-)	            Operador unário. Retorna a negação de seu operando.	        Se x é 3, então -x retorna -3.

    Adição (+)	            Operador unário. Tenta converter o operando em um número,
                            sempre que possível.	                                    +"3" retorna 3. +true retorna 1.

    Operador de
    exponenciação (**) 	    Calcula a base elevada á potência do expoente,
                            que é, baseexpoente	                                        2 ** 3 retorna 8. 10 ** -1 retorna 0.1
    ...
    Por exemplo, o número decimal nove possui uma representação binária 1001. Operadores bit a bit realizam suas operações nestas representações, mas retornam valores
    numéricos padrões do JavaScript.
    ...
    Deslocamento
    à esquerda	            a << b	        Desloca a em representação binária b bits à esquerda, preenchendo com zeros à direita.
    Deslocamento
    à direita com
    propagação de sinal	    a >> b	        Desloca a em representação binária b bits à direita, descartando bits excedentes.
    Deslocamento à
    direita com
    preenchimento zero	    a >>> b	        Desloca a em representação binária b bits à direita, descartando bits excedentes e preenchendo com zeros à esquerda.

    -----Operadores bit a bit lógicos-----
    Conceitualmente, os operadores bit a bit lógicos funcionam da seguinte maneira:
        -Os operandos são convertidos em inteiros de 32 bits e expressos como uma série de bits (zeros e uns). Números com representação maior que 32 bits terão
        seus bits truncados. Por exemplo, o seguinte inteiro tem representação binária maior que 32 bits será convertido em um inteiro de 32 bits.
    ...
    Por exemplo, a representação binária de nove é 1001 e a representação binária de quinze é 1111. Desta forma, quando operadores bit a bit são aplicados a estes valores,
    ...
    Nota: No quadro acima perceba que todos os 32 bits são invertidos quando usa-se o operador bit a bit NOT, e que os bits mais significativos (extrema esquerda) são
    definidos com 1 que representam valores negativos (representação complemento de dois).
 
    -----Operadores de deslocamento bit a bit-----
    Os operadores de deslocamento bit a bit possui dois operandos: o primeiro é uma quantidade a ser deslocada e o segundo especifica o número de posições binárias as
    quais o primeiro operando deverá ser deslocado. A direção da operação de deslocamento é contralada pelo operador utilizado.
    ...
    Operador	                    Descrição	                                                    Exemplo
    Deslocamento à esquerda (<<)	Este operador desloca o primeiro operando pelo número
                                    especificado de bits à esquerda. Bits excedentes
                                    deslocados para fora do limite à esquerda são
                                    descartados. Bits zero são inseridos à direita.	                9<<2 produz 36 porque 1001 deslocado 2 bits à esquerda se
                                                                                                    torna 100100, que é 36.
    Deslocamento à direita com
    propagação de sinal (>>)	    Este operador desloca o primeiro operando pelo número
                                    especificado de bits à direita. Bits excedentes
                                    deslocados para fora do limite à direita são descartados.
                                    Cópias dos bits mais à esquerda são deslocadas a partir
                                    da esquerda.	                                                9>>2 produz 2 porque 1001 deslocado 2 bits à direita se torna 10,
                                                                                                    que é 2. De forma similar, -9>>2 produz -3 porque o sinal é preservado.
    Deslocamento à direita
    com preenchimento zero (>>>)	Este operador desloca o primeiro operando pelo número
                                    especificado de bits à direita. Bits excedentes deslocados
                                    para fora do limite à direita são descartados.
                                    Bits zero são inseridos à esquerda.	                            19>>>2 produz 4 porque 10011 deslocado 2 bits à direita se torna 100,
                                                                                                    que é 4.
                                                                                                    Para números não negativos o deslocamento à direita com propagação de
                                                                                                    sinal e o
                                                                                                    deslocamento à direita com preenchimento zero produzem o mesmo resultado.

    -----Operadores lógicos-----
    Operadores lógicos são utilizados tipicamente com valores booleanos (lógicos); neste caso, retornam um valor booleano. Entretanto, os operadores && e || na
    verdade retornam o valor de um dos operandos especificados, de forma que se esses operadores forem utilizados com valores não-booleanos, eles possam retornar
    um valor não-booleano. Os operadores lógicos são descritos na seguinte tabela.
    Operadores lógico
    Operador	            Utilização	        Descrição
    AND lógico (&&)	        expr1 && expr2	    (E lógico) - Retorna expr1 caso possa ser convertido para falso; senão, retorna expr2. Assim, quando utilizado
                                                com valores booleanos, && retorna verdadeiro caso ambos operandos sejam verdadeiros; caso contrário, retorna falso.
    OU lógico (||)	        expr1 || expr2	    (OU lógico) -  Retorna expr1 caso possa ser convertido para verdadeiro; senão, retorna expr2. Assim, quando utilizado
                                                com valores booleanos, || retorna verdadeiro caso ambos os operandos sejam verdadeiro; se ambos forem falsos, retorna falso.
...
/*
    ...
    As regras de lógica garantem que estas avaliações estejam sempre corretas. Repare que a parte qualquercoisa das expressões acima não é avaliada, de forma que
    qualquer efeito colateral de fazê-lo não produz efeito algum.

    -----Operadores de string-----
    Além dos operadores de comparação, que podem ser utilizados em valores string, o operador de concatenação (+) concatena dois valores string, retornando outra string
...
/*
    ...
    Se condicao for verdadeira, o operador terá o valor de valor1. Caso contrário, terá o valor de valor2. Você pode utilizar o operador condicional em qualquer lugar
    onde utilizaria um operador padrão.
    Por exemplo,
    var status = (idade >= 18) ? "adulto" : "menor de idade";
*/
// var b_2 = 10;
// var b_3 = (b_2 === 10) ? console.log('true') : console.log('false'); /* if ternário */
/*
    ...
    -----Operador vírgula-----
    O operador vírgula (,) simplesmente avalia ambos de seus operandos e retorna o valor do segundo. Este operador é utilizado primariamente dentro de um laço for para
    permitir que multiplas variáveis sejam atualizadas cada vez através do laço.
    Por exemplo, se a é uma matriz bidimensional com 10 elementos em um lado, o código a seguir utiliza o operador vírgula para incrementar duas variáveis de uma só vez.
    O código imprime os valores dos elementos diagonais da matriz:
*/
...
/*
    ...
    Se o operador delete for bem-sucedido, ele define a propriedade ou elemento para undefined. O operador delete retorna verdadeiro se a operação for possível;
    ele retorna falso se a operação não for possível.
*/
// x = 42;
// var y = 43;
// meuobj = new Number();
...
/*
    ...
    Quando você apaga um elemento de um array, o tamanho do array não é afetado. Por exemplo, se você apaga a[3], o valor de a[4] ainda estará em a[4] e a[3] passa a
    ser undefined.
    Quando o operador delete remove um elemento do array, aquele elemento não pertence mais ao array. No exemplo a seguir, arvores[3] é removido com delete.
    Entretanto, arvores[3] ainda é endereçável e retorna undefined.
*/
...
// }
/*
    Se você quer que um elemento do array exista, mas tenha um valor indefinido, utilize a palavra-chave undefined em vez do operador delete. No exemplo a seguir,
    o valor undefined é atribuído a arvores[3], mas o elemento da matriz ainda existe:
*/
// var arvores = new Array("pau-brasil", "loureiro", "cedro", "carvalho", "sicômoro");
// arvores[3] = undefined;
// if (3 in arvores) {
...
// void expressao
/*
    O operador void especifica que uma expressão deve ser avaliada sem retorno de valor. expressao é uma expressão JavaScript que deve ser avaliada. Os parênteses
    em torno da expressão são opcionais, mas é uma boa prática utilizá-los.
    ...
    O código a seguir cria um link de hipertexto que não faz coisa alguma quando clicado pelo usuário. Quando o usuário clica no link, void(0) é avaliado como indefinido,
    que não tem efeito em JavaScript.
...
/*
    ...
    Utilize o instanceof quando você precisar confirmar o tipo de um objeto em tempo de execução. Por exemplo, ao capturar exceções você pode desviar para um código
    de manipulação de exceção diferente dependendo do tipo de exceção lançada.
    ...
    A precedência de operadores determina a ordem em que eles são aplicados quando uma expressão é avaliada. Você pode substituir a precedência dos operadores utilizando
    parênteses.
...
/*
    ...
    O operador de agrupamento ( ) controla a precedência de avaliação de expressões. Por exemplo, você pode substituir a precedência da divisão e multiplicação para que
    a adição e subtração sejam avaliadas primeiro.
*/
// var a = 1;
// var b = 2;
// var c = 3;
...
/*
    -----Comprehensions-----
    Comprehensions são uma característica experimental de JavaScript, marcada para ser inclusa
    em uma versão futura do ECMAScript. Existem duas versões de Comprehensions:

    [for (x of y) x] -> Comprehensions de array.
    Sintaxe
        [for (x of iterable) x]
        [for (x of iterable) if (condition) x]
        [for (x of iterable) for (y of iterable) x + y]
        Exemplo 1 - Array comprehensions simples:
            [for (i of [ 1, 2, 3 ]) i*i ]; 
            // [ 1, 4, 9 ]
            var abc = [ "A", "B", "C" ];
            [for (letters of abc) letters.toLowerCase()];
            // [ "a", "b", "c" ]
        Exemplo 2 - Array comprehensions com if:
            var years = [ 1954, 1974, 1990, 2006, 2010, 2014 ];
            [for (year of years) if (year > 2000) year]; // [ 2006, 2010, 2014 ]
            [for (year of years) if (year > 2000) if(year < 2010) year]; // [ 2006], the same as below:
            [for (year of years) if (year > 2000 && year < 2010) year]; // [ 2006]
        Exemplo 3 - Array comprehensions comparado a map e filter:
            Um modo fácil de entender a sintaxe de array comprehension é comparar com os métodos map e filter:
            var numbers = [ 1, 2, 3 ];
        ...
        Exemplo 4 - Array comprehensions com dois arrays:
    ...
    (for (x of y) y) -> gerador de comprehensions
    Sintaxe
        (for (x of iterable) x)
        (for (x of iterable) if (condition) x)
        (for (x of iterable) for (y of iterable) x + y)
        Exemplo 1 - Simple generator comprehensionsSection:
            (for (i of [1, 2, 3]) i * i ); // generator function which yields 1, 4, and 9
            [...(for (i of [1, 2, 3]) i * i )]; // [1, 4, 9]
            var abc = ['A', 'B', 'C'];
            (for (letters of abc) letters.toLowerCase()); // generator function which yields "a", "b", and "c"
        Exemplo 2 - Generator comprehensions with if statement:
            var years = [1954, 1974, 1990, 2006, 2010, 2014];
            (for (year of years) if (year > 2000) year); // generator function which yields 2006, 2010, and 2014
            (for (year of years) if (year > 2000) if (year < 2010) year); // generator function which yields 2006, the same as below:
            (for (year of years) if (year > 2000 && year < 2010) year); // generator function which yields 2006
        Exemplo 3 - Simple generator comprehensionsSection:
            An easy way to understand generator comprehension syntax, is to compare it with the generator function.
            Example 1: Simple generator.
                var numbers = [1, 2, 3]; // Generator function
                (function*() {
                for (let i of numbers) {
                    yield i * i;
                }
                })(); // Generator comprehension
                (for (i of numbers) i * i ); // Result: both return a generator which yields [1, 4, 9]
            ...
            Example 2: Using if in generator.
                ...
                (for (i of numbers) if (i < 3) i); // Result: both return a generator which yields [1, 2]
...
/*
    ...
    -----new-----
    Você pode utilizar o operador new para criar uma instância de um tipo de objeto definido pelo usuário ou de um dos tipos de objeto predefinidos: Array, Boolean,
    Date, Function, Image, Number, Object, Option, RegExp ou String. No servidor, você pode também utilizar DbPool, Lock, File ou SendMail. Utilize o operador new da
    seguinte forma:
*/
...
/*
    ...
    Exemplo: Se você tem um array e deseja criar um novo array com os elementos do array já existente sendo parte do novo array, a sintaxe do array não será suficiente
    e você terá de usar uma combinação de push, splice, concat, etc. Com a sintaxe spread, isto torna-se muito mais sucinto:
*/
